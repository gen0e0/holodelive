...
func play() -> void:
	...
	tween = create_tween()
	tween.tween_property(_label, "position:x", center_x, _dur(SLIDE_IN_DURATION)) \
	...
	tween = create_tween()
	tween.tween_property(_label, "position:x", out_x, _dur(SLIDE_OUT_DURATION)) \
