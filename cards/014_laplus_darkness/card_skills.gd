...
func _skill_0(ctx: SkillContext) -> SkillResult:
	...
	for id in ctx.state.deck:
		...
		if inst.card_id in HOLOX_IDS:
			...
			break
	if found_id == -1:
		# 見つからなかった → デッキはそのまま、シャッフルのみ
		ctx.state.deck.shuffle()
		return SkillResult.done()
	# 見つかったカードをステージにプレイし、デッキをシャッフル
	ctx.emit_cue(AnimationCue.make_card(found_id).move().from_deck().to_my_stage())
	ZoneOps.play_to_stage_from_zone(ctx.state, ctx.player, found_id, ctx.recorder)
	ctx.state.deck.shuffle()
	return SkillResult.done_and_trigger_play(found_id, ctx.player)
